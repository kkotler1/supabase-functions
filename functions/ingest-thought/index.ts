...
async function getEmbedding(text: string): Promise<number[]> {
  ...
  if (!d.data?.[0]?.embedding) {
    console.error("Embedding failed:", JSON.stringify(d));
...
Only extract what's explicitly there.` },
  ...
  try { return JSON.parse(d.choices[0].message.content); }
    ...
    console.error("Metadata parse failed:", JSON.stringify(d));
...
Deno.serve(async (req: Request): Promise<Response> => {
  try {
    // --- Slack retry dedup fix ---
    // Slack retries if it doesn't get 200 within ~3s.
    // Our embedding + metadata calls exceed that window, but the
    // original request succeeds — retries just create duplicates.
    const retryNum = req.headers.get("X-Slack-Retry-Num");
    if (retryNum) {
      const retryReason = req.headers.get("X-Slack-Retry-Reason");
      console.log(`Ignoring Slack retry #${retryNum} (reason: ${retryReason})`);
